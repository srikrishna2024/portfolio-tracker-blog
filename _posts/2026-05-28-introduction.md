...
This has one known side effect: Nuitka-compiled binaries trigger false positives on BitDefender and a handful of AV engines that share its signature database. BitDefender's Malware Research Team has since confirmed the file is clean and the detection will be removed in their next update. 59 out of 67 engines on VirusTotal are clean, including Windows Defender, Kaspersky, Sophos, and McAfee.
